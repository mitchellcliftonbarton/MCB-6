<script>
	...
	const handleSubmit = async () => {
		...
		if (subscribeConfirm.status === 'success') {
			...
			alert(
				"There was an error subscribing to the newsletter. Hit me up at mitchellcliftonbarton@gmail.com and I'll add you the old fashioned way :)"
			);
...
<form on:submit|preventDefault={handleSubmit} class="flex items-center">
	...
	<button
		type="submit"
		class="border-light-grey bg-light-purple lg:hover:bg-transparent lg:hover:border-black border px-4 py-3 rounded-2xl transition-colors duration-300"
		>{buttonText}</button
	>
</form>
